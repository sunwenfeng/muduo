...
EventLoop::EventLoop()
        : looping_(false),
          ...
          iteration_(0),
          threadId_(CurrentThread::tid()),   //eventloop对象所在线程
          ...
          currentActiveChannel_(NULL)
{
    ...
    if (t_loopInThisThread)//一个线程中只能有一个eventloop对象，检查eventloop对象指针
    {
...
void EventLoop::loop()   //eventloop循环，必须在IO线程执行
{
    assert(!looping_);
    assertInLoopThread();   //判断当前线程和eventloop对象创建时的线程是否一致，不一致则退出，进一步保证eventloop对象只能在一个线程中使用
    looping_ = true;
